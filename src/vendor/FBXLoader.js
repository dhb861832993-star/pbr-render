...
class FBXTreeParser {
	...
	parse() {
		...
		this.applyOrmTextures( textures, images, sceneGraph );

		return sceneGraph;

	}

	// PBRRender patch: many Blender-exported FBX files (e.g. tripo3d output)
	// carry a combined AO/Roughness/Metallic (ORM) texture in the Textures
	// node but never connect it to the material — some exporters (Blender
	// FBX) even drop the Texture node entirely and leave the ORM image as an
	// orphaned Video. three's stock loader drops unconnected textures, and
	// Phong/Lambert materials cannot render roughness/metalness anyway.
	// Here we (1) upgrade Lambert/Phong materials to MeshStandardMaterial,
	// preserving color/maps, (2) recover the ORM texture from the textures
	// map OR from orphaned Video nodes, and (3) attach it to
	// aoMap/roughnessMap/metalnessMap so the PBR viewport and the channel
	// inspector (R=AO, G=Roughness, B=Metallic) all work.
	applyOrmTextures( textures, images, sceneGraph ) {

		const ormNameRe = /(^|[_\-\.\s])(AoRm|ORM|ARM|RMA)([_\-\.\s]|$)|OcclusionRoughness|RoughnessMetallic|MetallicRoughness|ao_rough|rough_met|ao\.rough|arm_/i;

		let orm = null;

		// 1) 已由 parseTextures 创建的 ORM 纹理
		for ( const texture of textures.values() ) {

			if ( ormNameRe.test( texture.name || '' ) ) {
		...
		// 2) 兜底：孤立 Video 里的 ORM 数据（Blender 导出只有 Video 节点，
		//    没有 Texture 节点引用）。按 attrName 识别，用 parseImages 提取
		//    的 blob/文件名重新建纹理。
		if ( orm === null && fbxTree.Objects && fbxTree.Objects.Video ) {

			for ( const nodeID in fbxTree.Objects.Video ) {

				const video = fbxTree.Objects.Video[ nodeID ];
				if ( ! video || typeof video.attrName !== 'string' ) continue;
				if ( ! ormNameRe.test( video.attrName ) ) continue;
				...
				orm = this.textureLoader.load( source );
				orm.name = video.attrName;
				break;
		...
		sceneGraph.traverse( ( obj ) => {
			...
			for ( let i = 0; i < mats.length; i ++ ) {
				...
				if ( m.isMeshPhongMaterial || m.isMeshLambertMaterial ) {
					...
					std.normalScale.copy( m.normalScale );
					std.emissive.copy( m.emissive );
					...
					std.side = m.side;
					// ORM map drives the PBR channels; fall back to neutral values.
					std.roughness = 1;
					std.metalness = 1;
					mats[ i ] = std;
					changed = true;
			...
			for ( let i = 0; i < mats.length; i ++ ) {
				...
				if ( ! m.aoMap ) m.aoMap = orm;
				if ( ! m.roughnessMap ) m.roughnessMap = orm;
				if ( ! m.metalnessMap ) m.metalnessMap = orm;
				m.aoMapIntensity = 1;
				m.needsUpdate = true;
