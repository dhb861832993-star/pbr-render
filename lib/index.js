...
const MODEL_EXTS = new Set([".glb", ".gltf", ".bin", ".ktx2", ".hdr"]);
...
- Whenever a 3D model file (.glb/.gltf) is present or produced in the session — you generated it via an API, downloaded it, the user mentioned a path, or one appeared in the workspace — you PREEMPTIVELY call \`pbr_render\` and emit a \`pbr3d\` fence as part of the same reply. Do not ask "want me to preview it?" — just do it, then summarize alongside.
...
- model: required — path to a .glb/.gltf file (absolute, or relative to the session workspace)
...
- One model per fence. The model's own PBR textures (baseColor/normal/roughness/metallic/AO/emissive maps embedded in GLB, or sibling files for GLTF) render automatically.
...
function createPbrRenderTool(rootProvider) {
  return {
    name: "pbr_render",
    description:
      "Prepare a 3D model file for PBR preview: validates the path, verifies the extension (.glb/.gltf), and confirms the file is servable to the browser. Call this BEFORE emitting a ```pbr3d fence. Returns the confirmed absolute path and a servable URL the browser will fetch.",
    parameters: {
      type: "object",
      properties: {
        model: {
          type: "string",
          description: "Path to the model file. Absolute, or relative to the session workspace. Must end in .glb or .gltf."
        }
      },
      required: ["model"],
    ...
    async execute(args, exec) {
      ...
      if (ext !== ".glb" && ext !== ".gltf") {
        return { ok: false, error: `unsupported model extension "${ext}" — use .glb or .gltf` };
      }
