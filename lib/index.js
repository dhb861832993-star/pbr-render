...
/** Asset route names: three.js bundle at the root, HDR env maps under env/. */
const ASSET_FILE_RE = /^(?:env\/)?[a-zA-Z0-9._-]+$/;
...
- env: one of the built-in HDR environment maps: "studio"|"sunset"|"outdoor"|"sunrise"|"night" (default "studio") — real CC0 HDR images (Poly Haven, 1K) driving IBL reflections/lighting
- envBackground: boolean (default false) — also show the HDR environment as the scene background instead of the plain background color
...
function createAssetHandler() {
  return async (req, res) => {
    ...
    try {
      ...
      res.writeHead(200, {
        "content-type": ext === ".hdr" ? "image/vnd.radiance" : "text/javascript; charset=utf-8",
        "cache-control": "no-cache"
      });
      res.end(req.method === "HEAD" ? undefined : body);
    } catch {
